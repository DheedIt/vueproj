<template>
    <div>
        ...
        <button v-if="isUploadDataAvivable" @click="addUsers(); disableDataUpload()">Add Users</button>
        <table>
...
export default {
    ...
    data() {
        return {
            users,
            columns,
            isUploadDataAvivable: enableUploadData
        }
    },
    methods: {
        addUsers() {
            this.users = this.users.concat(templateUser);
        },
        disableDataUpload() {
            this.isUploadDataAvivable = false;
        },
    }
}
</script>
